...
if ( hostname == "" or ' ' in hostname ):
    ...
    if ( requests.get(f"http://{hostname}/config.dns").status_code == 200 ):
        ...
        if ( requests.get(f"http://{hostname}/certificate.ttp").status_code == 200 ):
            ...
            valid_base = requests.get(f"http://{hostname}/valid_certificates.ttp").text
                    ...
                    if ( url == "" or ' ' in url ):
                        ...
                        for x2 in range(0, len(list2)):
                            if url in list2[x2]:
                                ...
                                if f'{url}/' in url:
                                    h = {'User-Agent': f'mouseTor/{version1}.{version2}.{version3} (Windows NT {os_Version}.0; Win{windows_x64_or_not}; x{windows_x64_or_not}; rv:{fake_rv}.0) mouseTor Relay/A9H8G88F mouseTor/1.0.0'}
